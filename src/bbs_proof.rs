...
impl BbsProof {
    ...
    pub fn prove(
        ...
        let l = messages.len();
        if disclosed.len() != l {
            return Err(ActError::ProtocolError("disclosed mask length mismatch".into()));
        }
        if l > 5 {
            return Err(ActError::ProtocolError("EBUT supports at most 5 hidden messages in this integrated layout".into()));
        }
    ...
    pub fn verify(
        ...
        if l > 5 {
            return Err(ActError::VerificationFailed("Too many messages (max 5)".into()));
        }
        if self.z_m_tilde.len() != l {
            return Err(ActError::VerificationFailed("message response length mismatch".into()));
        }
