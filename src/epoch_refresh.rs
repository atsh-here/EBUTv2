...
pub struct RefreshProof {
    ...
    /// Public BLS commitment to the hidden master secret x: x*h1 + r_x*h0.
...
pub struct RefreshClient {
    pub k_sub: Scalar,
    pub c_max: u32,
    pub e_max: u64, // unique Unix-second expiry/revocation handle
    pub t: u32,
    pub k_daily: Scalar,
    pub r_daily: Scalar,
    pub r_delta: Scalar,
...
impl RefreshProver {
    ...
    pub fn prove(
        ...
        // Non-transferable daily commitment:
        // K_daily = x*h1 + k_daily*h2 + cmax*h3 + T*h4 + Emax*h5 + r_daily*h0.
        // The hidden x and Emax are carried forward into every daily/refund token,
        // so the token cannot be spent without the master secret x.
        ...
        let k_daily_commit = &(&(&(&(&generators.h[1] * &k_sub.0)
            + &(&generators.h[2] * &k_daily.0))
            + &(&generators.h[3] * &Scalar::from(c_max).0))
            + &(&generators.h[4] * &Scalar::from(t).0))
            + &(&(&generators.h[5] * &Scalar::from(e_max).0)
            + &(&generators.h[0] * &r_daily.0));

        // Expiry commitment
        let r_delta = Scalar::rand(&mut fast_rng);
        let c_delta = &(&generators.h[5] * &Scalar::from(delta).0) + &(&generators.h[0] * &r_delta.0);

        // BLS commitment to the same hidden x, proven in this refresh proof.
        // Upload same-x bridging can use this commitment, or the spend proof's one.
        ...
        let msg_part = g1_msm(
            &[generators.g1_affine, generators.h_affine[0], generators.h_affine[1],
              generators.h_affine[3], generators.h_affine[5]],
            &[BlsScalar::ONE, master.s.0, k_sub.0,
              BlsScalar::from(c_max as u64), BlsScalar::from(e_max)],
        );
        ...
        let t_bbs = g1_msm(
            &[a_prime_aff, generators.g1_affine, generators.h_affine[0],
              generators.h_affine[1], generators.h_affine[3], generators.h_affine[5]],
            &[(-rho_e).0, rho_r1.0, rho_s.0, rho_k.0, rho_c.0, rho_e_msg.0],
        );

        // External blinders
        let (rho_u, rho_v, rho_w, rho_rx) = (
        ...
        let t_d = g1_msm(
            ...
            &[rho_k.0, rho_u.0, rho_c.0,
              (rho_r1 * Scalar::from(t)).0, rho_e_msg.0, rho_v.0],
        );
        let c_bridge  = &c_delta + &(&generators.h[5] * &Scalar::from(now_unix).0);
        ...
        let (batched_eq, c_delta_from_beq) = prove_batched_equality(
            &mut fast_rng, delta, r_delta.0, generators.h[5], generators.h[0], &beq_ctx, &beq_commitments,
        )?;
        ...
        let c = Self::challenge(
            h_ctx, pk_master, t, now_unix, n_t, k_daily_commit, c_delta,
            x_bls_commitment, &beq_bytes, a_prime, a_bar, t_bbs,
            t_scale_n, t_n, t_scale_d, t_d, t_scale_b, t_b, t_scale_x, t_x,
        );
...
pub fn verify_refresh(
    ...
    let c_bridge = &proof.c_delta + &(&generators.h[5] * &Scalar::from(now_unix).0);
    let beq_bytes = proof.batched_eq.to_bytes();

    let c = RefreshProver::challenge(
        h_ctx, pk_master, current_epoch, now_unix,
        proof.n_t, proof.k_daily, proof.c_delta, proof.x_bls_commitment,
        &beq_bytes,
        proof.a_prime, proof.a_bar, proof.t_bbs,
        proof.t_scale_n, proof.t_n, proof.t_scale_d, proof.t_d,
        proof.t_scale_b, proof.t_b, proof.t_scale_x, proof.t_x,
    );
    let c_fr = c.0;

    // Explicit bridge: the public BLS x commitment opens to the same hidden x
    // used inside the BBS+ possession proof.
    let lhs_x = &(&generators.h[1] * &proof.z_k_tilde.0) + &(&generators.h[0] * &proof.z_r_x.0) - &proof.t_x;
    ...
    // Schwartz–Zippell RLC combined MSM check (soundness ≤ 4/|Fr|).
    {
        ...
        let sc_tn       = -c_fr;
        // Combined equation E1 + c*E2 + c^2*E3 + c^3*E4 after adding
        // x and Emax into the daily token commitment.
        let sc_h0       = &(&c2 * &proof.z_v.0) + &(&(&c3 * &proof.z_w.0) + &proof.z_s_tilde.0);
        let sc_h1       = &(&c2 + &BlsScalar::ONE) * &proof.z_k_tilde.0;       // x in master + daily
        let sc_h2       = &c2 * &proof.z_u.0;                                  // k_daily
        let sc_h3       = &(&c2 + &BlsScalar::ONE) * &proof.z_c_tilde.0;       // cmax
        let sc_h4       = &c2 * &(&proof.z_r1.0 * &ep);                       // epoch T
        let sc_h5       = &(&BlsScalar::ONE + &(&c2 + &c3)) * &proof.z_e_tilde.0; // Emax in master + daily + expiry
...
/// Public BLS commitment to the hidden Emax proven by [`RefreshProof`].
///
/// The refresh proof contains `c_delta = (Emax - now_unix)*h5 + r*h0`;
...
pub fn refresh_emax_bls_commitment(
    proof: &RefreshProof,
    now_unix: u64,
    generators: &Generators,
) -> G1Projective {
...
pub fn verify_refresh_batch(
    ...
    // Safety-first integration path: the old optimized batch verifier was tied
    // to the pre-non-transferable message layout. Until a fresh RLC derivation
    // is audited, batch verification falls back to individual verification.
    proofs
        .iter()
        .map(|proof| verify_refresh(proof, current_epoch, now_unix, generators, pk_master, keys, h_ctx, rng))
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn refresh_roundtrip_nontransferable_with_x_commitment() {
        ...
        let (client, proof) = RefreshProver::prove(
            &mut rng, &master, k_sub, c_max, e_max, epoch, now, &generators, &keys.pk_master, h_ctx,
        ).unwrap();
        let response = verify_refresh(&proof, epoch, now, &generators, &keys.pk_master, &keys, h_ctx, &mut rng).unwrap();
        let (daily_sig, k_daily) = client.finalize(response);
        assert!(!bool::from(daily_sig.a.is_identity()));
        assert!(!k_daily.is_zero());
        assert!(!bool::from(proof.x_bls_commitment.is_identity()));
    }
    ...
    fn expired_refresh_rejected() {
        ...
        let h_ctx = compute_h_ctx("test-app", &keys.pk_master, &keys.pk_daily, &generators);
        let k_sub = Scalar::rand_nonzero(&mut rng);
        let master = create_master_sig(&mut rng, k_sub, 100, 1_000, &generators, &keys);
        assert!(RefreshProver::prove(
            &mut rng, &master, k_sub, 100, 1_000, 10, 2_000, &generators, &keys.pk_master, h_ctx,
        ).is_err());
    }

    #[test]
    fn tampered_x_commitment_fails() {
        let mut rng = thread_rng();
        let generators = Generators::new();
        let keys = ServerKeys::generate(&mut rng);
        let h_ctx = compute_h_ctx("test-app", &keys.pk_master, &keys.pk_daily, &generators);
        let k_sub = Scalar::rand_nonzero(&mut rng);
        let master = create_master_sig(&mut rng, k_sub, 100, 5_000_000_000, &generators, &keys);
        let (_client, mut proof) = RefreshProver::prove(
            &mut rng, &master, k_sub, 100, 5_000_000_000, 10, 1_700_000_000, &generators, &keys.pk_master, h_ctx,
        ).unwrap();
        proof.x_bls_commitment += generators.h[1];
        assert!(verify_refresh(&proof, 10, 1_700_000_000, &generators, &keys.pk_master, &keys, h_ctx, &mut rng).is_err());
    }
}
