...
use crate::same_x_bridge::CanonicalX;
...
impl MasterMintClient {
    pub fn begin(
        ...
        let canonical_x = CanonicalX::random(rng);
        let k_sub = canonical_x.to_ebut_scalar();
        ...
        (
            Self { k_sub, r_sub, c_max, e_max, canonical_x },
            MasterMintRequest { k_sub_commit, pok },
        )
    }

    /// Begin minting with an externally supplied canonical ownership secret.
    pub fn begin_with_x(
        rng: &mut (impl RngCore + CryptoRng),
        canonical_x: CanonicalX,
        c_max: u32,
        e_max: u64,
        generators: &Generators,
        h_ctx: Scalar,
...
impl MasterMintServer {
    ...
    pub fn issue(
        ...
        let bases  = [
            ...
            k_sub_aff,              // h1*x + h0*r_sub from the client's commitment
            generators.h_affine[0], // server-side signature blinding
            generators.h_affine[3], // c_max
            generators.h_affine[5], // Emax unique Unix-second revocation/expiry handle
        ];
        let scalars = [
            Scalar::ONE.0,
            Scalar::ONE.0,
