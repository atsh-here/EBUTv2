...
fn now_unix() -> Result<u64> {
    SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .map_err(|_| ActError::ProtocolError("system clock before Unix epoch".into()))
}
...
impl NullifierManager {
    ...
    pub async fn check_spend_nullifier(&self, k_cur: Scalar, t_issue: u32) -> Result<bool> {
        ...
        } else if self.config.current_epoch > 0 && t_issue == self.config.current_epoch - 1 {
...
impl NonceManager {
    ...
    pub async fn generate_nonce(&self) -> Result<[u8; 16]> {
        // Generate only. Do not pre-store it in Redis: verify_and_consume uses
        // SETEX-if-absent semantics, so pre-storing here would make the freshly
        // generated nonce look already spent.
...
pub mod handlers {
    ...
    pub async fn handle_refresh(
        ...
        let response = tokio::task::spawn_blocking(move || {
            let mut rng = rand::thread_rng();
            verify_refresh(&proof, current_epoch, now, &generators, &pk_master, &keys, h_ctx, &mut rng)
        })
        .await
    ...
    pub async fn handle_spend(
        ...
        let response = tokio::task::spawn_blocking(move || {
            let mut rng = rand::thread_rng();
            verify_spend(&proof, current_epoch, now, &nonce_copy, &generators, &pk_daily, &keys, h_ctx, &mut rng)
        })
        .await
        .map_err(|e| ActError::ProtocolError(format!("Blocking task panicked: {e}")))?
        ?;
